"""Shared statistics aggregation logic.

The statistics computation lives here so that both the authenticated
statistics router and the public profile endpoint can reuse it for any user,
without coupling the public data path to FastAPI auth dependencies.
"""
...
from app.models import AcquisitionStatus, Author, Book, BookAuthor, Medium, ReadingProgress, ReadingStatus, UserSettings
...
def _compute_goal_progress(
    ...
    Disabled goals are omitted from the result; the dashboard only shows
    ...
    # Mirror compute_statistics: anchor every book with progress at page 0 on
    # its start date so the first progress delta is attributed to the reading
    # span, keeping the pages-per-month goal consistent with the statistics
    # chart.
    virtual_entries = [
    ...
    return _goal_results(
        tz, today_key, current_month_key, current_year,
        entries, books, fallback_books, virtual_entries, settings,
    )
...
def _goal_results(
    ...
    """Assemble enabled goal progress entries."""
    goals_spec = [
...
def compute_statistics(
    session: Session,
    user_id: int,
    range_value: StatisticsRange = StatisticsRange.alltime,
    custom_from: Optional[date] = None,
    custom_to: Optional[date] = None,
) -> StatisticsResponse:
    """Compute the full statistics dashboard for *user_id*.

    Mirrors the authenticated ``/api/statistics`` endpoint so the public
    profile can reuse the exact same aggregation for the link owner.
    """
    if range_value == StatisticsRange.custom:
        if custom_from is None or custom_to is None:
            raise HTTPException(
    ...
    books = list(session.exec(select(Book).where(Book.user_id == user_id)).all())
